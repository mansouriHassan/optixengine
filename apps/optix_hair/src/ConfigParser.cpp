...
void ConfigParser::parseConfigData(const std::string& json_data)
{
    ...
    while (true) {
        if (!json_data.empty())
        {
            ...
            assert(shadeColor1["HT"].IsString());
            printf("HT = %s\n", shadeColor1["HT"].GetString());

            config_json = config_json + "HT : " + shadeColor1["HT"].GetString() + "\n";
            ...
            assert(shadeColor2["HT"].IsString());
            printf("HT = %s\n", shadeColor2["HT"].GetString());
            config_json = config_json + "HT : " + shadeColor2["HT"].GetString() + "\n";
